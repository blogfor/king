...
$LastChangedDate: 2015-05-16 12:09:51 -0700 (Sat, 16 May 2015) $
$Rev: 12891 $
*/
	...
	foreach ($cOrder as $c) {
		switch($c) {
			...
			if ($spDevice == 'mobile' && array_key_exists('iconMobileSubmit', $a) && !empty($a['iconMobileSubmit'])) {
				# display mobile icon
				$out.= "<input type='image' src='".sp_paint_file_icon(SPTHEMEICONSURL, "$iconMobileSubmit")."' class='spIcon' name='newtopic' id='sfsave' />";
			} else {
			...
			if ($spDevice == 'mobile' && array_key_exists('iconMobileCancel', $a) && !empty($a['iconMobileCancel'])) {
				# display mobile icon
				$out.= "<img src='".sp_paint_file_icon(SPTHEMEICONSURL, "$iconMobileCancel")."' class='spIcon' name='cancel' id='sfcancel' onclick='if (confirm(\"".$msg."\")) {spjEdCancelEditor();}' />";
			} else {
				# display default button
...
function sp_post_editor_submit_buttons($out, $spThisTopic, $a, $toolbar) {
	...
	foreach ($cOrder as $c) {
		switch($c) {
			...
			if ($spDevice == 'mobile' && array_key_exists('iconMobileSubmit', $a) && !empty($a['iconMobileSubmit'])) {
				# display mobile icon
				$out.= "<input type='image' src='".sp_paint_file_icon(SPTHEMEICONSURL, "$iconMobileSubmit")."' class='spIcon' name='newpost' id='sfsave' />";
			} else {
				# display default button
