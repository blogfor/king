...
$LastChangedDate: 2015-12-14 18:04:44 -0800 (Mon, 14 Dec 2015) $
$Rev: 13693 $
*/
...
function sp_OpenCloseControl($args='', $toolTipOpen='', $toolTipClose='') {
	...
	$out = "<a id='$tagId' class='$linkClass' href='javascript:void(0);' onclick='spjOpenCloseSection(\"$targetId\", \"$tagId\", \"$tagClass\", \"$openIcon\", \"$closeIcon\", \"$toolTipOpen\", \"$toolTipClose\", \"$setCookie\", \"$asLabel\", \"$linkClass\");'>";
	if($asLabel) {
		$out.= $toolTip;
	} else {
...
function sp_UserMembership($args='', $memberships) {
	...
	$defs = array('tagClass'	=> 'spUserMemberships',
	...
	if ($mobileMenu) {
		...
		for ($x=0; $x<strlen($order); $x++) {
			...
			if ($unreads > 0 && $item != 'T') {
				...
				if ($item == 'M') {
					$out.= "<a href='javascript:void(null)' onclick='spjMarkRead(\"$ajaxUrl\");'>".sp_paint_icon($iconClass, SPTHEMEICONSURL, $markIcon, $markToolTip)."</a>";
				}
			}
		}
...
function sp_UnreadPostsLink($args='', $label='', $unreadToolTip='', $popupLabel='') {
	...
	if ($popup) {
		$site = SFHOMEURL."index.php?sp_ahah=newpostpopup&amp;action=all&amp;first=$first&amp;group=$group&amp;count=$count&amp;sfnonce=".wp_create_nonce('forum-ahah');
		$out.= "<a rel='nofollow' id='$tagId' class='$tagClass' title='$unreadToolTip' href='javascript:void(null)' onclick='spjDialogAjax(this, \"$site\", \"$popupLabel\", 700, 500, \"center\");'>$label</a>";
	} else {
		$args = array();
		$args['first'] = $first;
		$args['group'] = $group;
...
function sp_MarkReadLink($args='', $label='', $markToolTip='') {
	...
	$out.= "<a id='$tagId' class='$tagClass' title='$markToolTip' href='javascript:void(null)' onclick='spjMarkRead(\"$ajaxUrl\");'>$label</a>";
...
function sp_MarkForumReadMobile($args='', $label='', $text='') {
	...
	$ajaxUrl = SFHOMEURL.'index.php?sp_ahah=newpostpopup&amp;sfnonce='.wp_create_nonce('forum-ahah')."&amp;action=mark-forum-read&amp;forum=".$spVars['forumid'];
	$out.= "<p><a class='$buttonClass' href='javascript:void(null)' onclick='spjMarkForumRead(\"$ajaxUrl\", $count); jQuery(\"#$tagId\").slideUp(); spjResetMobileMenu();'>$label</a></p>";
...
function sp_MarkForumRead($args='', $label='', $markToolTip='') {
	...
	if (!empty($markIcon)) {
		if (!$mobileMenu) $label = '';
		$out.= "<a href='javascript:void(null)' onclick='spjMarkForumRead(\"$ajaxUrl\", $count);'>";
