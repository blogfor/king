    ...
    jQuery(document).ready(function(){
    	...
    	new AjaxUpload(button,{
    		...
    		onComplete: function(file, response){
    			...
    			if (response==="success"){
                    var site = "<?php echo SFHOMEURL; ?>index.php?sp_ahah=components&amp;sfnonce=<?php echo wp_create_nonce('forum-ahah'); ?>&amp;action=delbadge&amp;file=" + file;
    				var count = document.getElementById('rank-count');
    				var rcount = parseInt(count.value) + 1;
    				jQuery('#sf-rank-badges').append('<tr id="rank' + rcount + '" class="spMobileTableData"><td data-label="<?php spa_etext('Badge'); ?>"><img class="sfrankbadge" src="<?php echo SFRANKS; ?>/' + file + '" alt="" /></td><td  data-label="<?php spa_etext('Filename'); ?>">' + file + '</td><td data-label="<?php spa_etext('Remove'); ?>"><img src="<?php echo SFCOMMONIMAGES; ?>' + 'delete.png' + '" title="<?php echo esc_js(spa_text('Delete Rank Badge')); ?>" alt="" onclick="spjDelRow(\'' + site + '\', \'rank' + rcount + '\');" /></td></tr>');
