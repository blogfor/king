...
$LastChangedDate: 2015-08-03 19:56:36 -0700 (Mon, 03 Aug 2015) $
$Rev: 13240 $
*/
    ...
    jQuery(document).ready(function(){
    	...
    	new AjaxUpload(button,{
    		...
    		onComplete: function(file, response){
    			...
    			if (response==="success"){
                    var site = "<?php echo SFHOMEURL; ?>index.php?sp_ahah=components&amp;sfnonce=<?php echo wp_create_nonce('forum-ahah'); ?>&amp;action=delsmiley&amp;file=" + file;
    				...
    				jQuery('#sfsmileytable').append(
                        '<tr id="smiley' + scount + '">' +
                        '<td class="dragHandle" style="text-align:center;width:2%"><img class="spSmiley" src="<?php echo SFSMILEYS; ?>' + file + '" alt="" /></td>' +
                        '<td class="wp-core-ui">' +
                        '<fieldset class="sffieldset"><legend><strong>' + file + '</strong></legend><input type="hidden" name="smfile[]" value="' + file + '" />' +
                        '<div class="sp-form-row"><div class="wp-core-ui sflabel sp-label-40"><?php echo spa_text('Name'); ?>:</div><input type="text" class="wp-core-ui sp-input-60" name="smname[]" value="' + name + '" /><div class="clearboth"></div></div>' +
                        '<input type="hidden" name="smfile[]" value="' + file + '" />' +
                        '<div class="sp-form-row"><div class="wp-core-ui sflabel sp-label-40"><?php echo spa_text('Code'); ?>:</div><input type="text" class="wp-core-ui sp-input-60" name="smcode[]" value="' + name + '" /><div class="clearboth"></div></div>' +
                        '<div class="sp-form-row"><input type="checkbox" name="smbreak-' + name + '" id="sf-smbreak-' + name + '" /><label for="sf-smbreak-' + name + '" class="wp-core-ui"><?php echo spa_text('Break Smileys Row in Editor Display'); ?></label><div class="clearboth"></div></div>' +
                        '<div class="sp-form-row"><input type="checkbox" name="sminuse-'+ name + '" id="sf-sminuse-' + name + '" checked="checked" /><label for="sf-sminuse-' + name + '" class="wp-core-ui"><?php echo spa_text('Allow Use of this Smiley'); ?></label><div class="clearboth"></div></div>' +
                        '</fieldset>' +
                        '</td>' +
                        '<td style="text-align: left;vertical-align: middle;margin:0;padding:0 4px 0 0;width:3%"><img src="<?php echo SFCOMMONIMAGES.'delete.png'; ?>" title="<?php echo spa_text('Delete Smiley'); ?>" alt="" onclick="spjDelRow(\'' + site + '\', \'smiley' + scount + '\');" /></td>' +
                        '</tr>');
