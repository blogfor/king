...
$LastChangedDate: 2015-09-16 00:41:55 -0700 (Wed, 16 Sep 2015) $
$Rev: 13469 $
*/
...
class spPost {
	...
	function validateHuman($postVars) {
		...
		if (empty($postVars['forumid']) || empty($postVars['action']) || empty($postVars['postitem'])) {
			$this->abort = true;
			return;
		}

		# check the WP nonce
		if ($postVars['action'] == 'topic') {
		...
		if ($postVars['action'] == 'topic' && empty($postVars['newtopicname']) || ($postVars['action'] == 'post' && empty($postVars['topicid']))) {
			$this->abort = true;
			return;
		}
		if ($this->guest) {
